...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // GSAP Animations for Even/Odd Switch
  // Animate the even/odd panel entrance when it becomes visible
  const evenOddCard = document.querySelector("#even-odd-panel .smart-card");
  if (evenOddCard) {
    // Create observer for when the panel becomes visible
    const observer = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        if (mutation.type === 'attributes' && mutation.attributeName === 'style') {
          const evenOddContainer = document.getElementById("even-odd-panel");
          if (evenOddContainer.style.display !== "none") {
            // Animate card entrance with 3D flip effect for symmetry
            gsap.fromTo(evenOddCard, 
              { 
                opacity: 0, 
                scale: 0.7, 
                rotationX: 45,
              ...
              { 
                ...
                onComplete: () => {
                  ...
                  const headerElements = evenOddCard.querySelectorAll('.smart-title, .smart-sub');
                  ...
                  // Animate tick display with special effect
                  const tickDisplay = evenOddCard.querySelector('.tick-display');
                  if (tickDisplay) {
                    gsap.fromTo(tickDisplay, 
                  ...
                  // Animate form elements with symmetric stagger
                  const formElements = evenOddCard.querySelectorAll('.field, .stake-row, .smart-results');
    ...
    observer.observe(document.getElementById("even-odd-panel"), { attributes: true });
  }

  // Add animations for tick grid items
  const animateTickItem = (tickItem, isNew = false) => {
    if (isNew) {
      // Special animation for new ticks
      gsap.fromTo(tickItem, 
        { 
          scale: 0, 
          rotation: 180,
          opacity: 0 
      ...
      // Gentle entrance animation for existing ticks
      gsap.fromTo(tickItem, 
        { opacity: 0, scale: 0.8 },
        { 
          duration: 0.4,
  ...
  // Override updateTickDisplay to add animations
  const originalUpdateTickDisplay = window.updateTickDisplay;
  window.updateTickDisplay = function() {
    if (originalUpdateTickDisplay) originalUpdateTickDisplay();
    
    // Add animations to tick items
    const tickItems = document.querySelectorAll("#tick-grid-eo .tick-item");
    tickItems.forEach((tickItem, index) => {
      const isNew = tickItem.classList.contains('new-tick');
      setTimeout(() => {
        animateTickItem(tickItem, isNew);
      }, index * 20); // Stagger animation for wave effect
    });
  };

  // Add hover animations for tick items
  ...
  // Add animations for the RUN button
  const runBtn = document.getElementById("run-even-odd");
  if (runBtn) {
    runBtn.addEventListener("mouseenter", () => {
      gsap.to(runBtn, {
        ...
        scale: 1.05,
        rotation: 2,
        boxShadow: "0 10px 30px rgba(0, 187, 240, 0.4)",
        ease: "power2.out"
      });
    });
    ...
    runBtn.addEventListener("mouseleave", () => {
      gsap.to(runBtn, {
        ...
        scale: 1,
        rotation: 0,
        boxShadow: "0 4px 15px rgba(2,6,23,0.1)",
        ease: "power2.out"
      });
    });
  ...
  document.querySelectorAll("#even-odd-panel input").forEach(input => {
  ...
  // Add animation for tick count display
  const tickCountDisplay = document.getElementById("total-ticks-eo");
  if (tickCountDisplay) {
    const originalTextContent = Object.getOwnPropertyDescriptor(HTMLSpanElement.prototype, 'textContent').set;
    Object.defineProperty(tickCountDisplay, 'textContent', {
      set: function(value) {
